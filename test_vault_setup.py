...
def test_vault_decryption():
    ...
    try:
        ...
        view_result = run_command(
            f"ansible-vault view {secrets_file} --vault-password-file {temp_file_path}"
        )
        ...
        try:
            ...
            masked_creds = {
                k: v[:2] + "*****" if k.endswith("password") else v
...
def main():
    ...
    print(
        f"CI Secrets Extraction Test: {'✅ PASSED' if ci_extraction_test else '❌ FAILED'}"
    )
